...
export default function App() {
  ...
  // Initialize blocked state to handle age restriction
  const [blocked, setBlocked] = useState(false);

  // Enforce age restriction whenever user data changes
  useEffect(() => {
  ...
  // If age restriction blocked, render access denied message
  if (blocked) {
    return (
  ...
  // existing logic continues below (original code will be placed after this block)
  // ...
};
e.preventDefault();
if (!formData.name.trim() || !formData.age) return toast.error("Name and Age are required!");
...
// Age restriction check before rendering UI
if (user && Number(user.age) < 18) {
  window.alert("Access denied: you must be at least 18 years old to use this site.");
  sessionStorage.clear();
  return (
              ...
              {activeUsers.filter(u => (genderFilter === 'All' || u.gender === genderFilter) && (countryFilter === 'All' || u.country === countryFilter)).map(u => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-2xl cursor-pointer flex items-center justify-between transition-all duration-300 border ${isSelected
                          ...
                          {isEditingThis ? (
                            ...
                            <div className="flex flex-col relative group pb-1">
                              ...
                              <div
                                ...
                                className={`p-3.5 px-4 rounded-2xl text-sm shadow-md cursor-pointer relative transition-all duration-200 ${isMyMsg
